import { GraduationCap } from 'lucide-react'
import { useAuth } from '@/contexts/AuthContext'
import { Button } from '@/components/ui/Button'

export function LoginPage() {
  const { signInWithGoogle } = useAuth()

  return (
    <div className="min-h-screen bg-gradient-to-br from-primary-600 via-primary-700 to-primary-900 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md text-center">
        {/* Logo */}
        <div className="flex justify-center mb-6">
          <div className="w-16 h-16 bg-primary-600 rounded-2xl flex items-center justify-center shadow-lg">
            <GraduationCap size={32} className="text-white" />
          </div>
        </div>

        {/* Branding */}
        <h1 className="text-3xl font-bold text-slate-900 mb-2">Prof. Raposo</h1>
        <p className="text-slate-500 mb-8 leading-relaxed">
          Seu assistente inteligente para planejamento de aulas,<br />
          geração de atividades e organização escolar.
        </p>

        {/* Features list */}
        <div className="text-left space-y-2 mb-8 bg-slate-50 rounded-xl p-4">
          {[
            '✨ Planos de aula em segundos com IA',
            '📊 Slides profissionais gerados automaticamente',
            '📅 Calendário letivo integrado',
            '📚 Acervo de todos os seus materiais',
          ].map((item) => (
            <p key={item} className="text-sm text-slate-600">{item}</p>
          ))}
        </div>

        {/* Google sign-in */}
        <Button
          onClick={signInWithGoogle}
          size="lg"
          className="w-full"
          icon={
            <svg viewBox="0 0 24 24" className="w-5 h-5" fill="currentColor">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#fff" opacity=".7"/>
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#fff" opacity=".7"/>
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#fff" opacity=".7"/>
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#fff" opacity=".7"/>
            </svg>
          }
        >
          Entrar com Google
        </Button>

        <p className="text-xs text-slate-400 mt-4">
          Ao entrar, você concorda com o uso seguro dos seus dados escolares.
        </p>
      </div>
    </div>
  )
}
